...
fn lotr() -> Book {
    Book::new().with_title("Il Signore degli Anelli").with_npages(1000).with_read_pages(247).with_cover_path("lotr.jpg").with_author("J.R.R. Tolkien").with_description("Il Signore degli Anelli è romanzo d'eccezione, al di fuori del tempo: chiarissimo ed enigmatico, semplice e sublime. \
    Dona alla felicità del lettore ciò che la narrativa del nostro secolo sembrava incapace di offrire: avventure in luoghi remoti e terribili, episodi d'inesauribile allegria, segreti paurosi che si svelano a poco a poco, draghi crudeli e alberi che camminano, città d'argento e di diamante poco lontane da necropoli tenebrose in cui dimorano esseri che spaventano solo al nominarli, urti giganteschi di eserciti luminosi e oscuri; e tutto questo in un mondo immaginario ma ricostruito con cura meticolosa, e in effetti assolutamente verosimile, perché dietro i suoi simboli si nasconde una realtà che dura oltre e malgrado la storia: la lotta, senza tregua, fra il bene e il male.\
...
fn _1984() -> Book {
    Book::new().with_title("1984").with_npages(300).with_read_pages(37).with_cover_path("1984.jpg").with_author("George Orwell").with_description("1984.\
     Il mondo è diviso in tre immensi superstati in perenne guerra fra loro: Oceania, Eurasia ed Estasia.\
      In Oceania la società è governata dall'infallibile e onnisciente Grande Fratello, che nessuno ha mai visto di persona.
       I suoi occhi sono le telecamere che spiano di continuo nelle case, il suo braccio la Psicopolizia che interviene al minimo sospetto.\
        Non c'è legge scritta e niente, apparentemente, è proibito.\
         Tranne divertirsi. Tranne pensare. Tranne amare. Tranne vivere, insomma. \
...
fn sotto_lo_stesso_cielo() -> Book {
    Book::new().with_title("Sotto lo stesso cielo").with_npages(252).with_read_pages(252).with_cover_path("sotto-lo-stesso-cielo.jpg").with_author("Giulia Pompili").with_description("
...
fn california() -> Book {
    Book::new().with_title("California: La Fine del Sogno").with_npages(204).with_read_pages(0).with_cover_path("california-la-fine-del-sogno.jpg").with_author("Francesco Costa").with_description("\
...
fn farenheit() -> Book {
    Book::new().with_title("Farhrenheit 451").with_npages(177).with_read_pages(52).with_cover_path("451.jpg").with_author("Ray Bradbury").with_description("\
    Montag fa il pompiere in un mondo in cui ai pompieri non è richiesto di spegnere gli incendi, ma di accenderli: armati di lanciafiamme, fanno irruzione nelle case dei sovversivi che conservano libri e li bruciano. Così vuole fa legge. Montag però non è felice della sua esistenza alienata, fra giganteschi schermi televisivi, una moglie che gli è indifferente e un lavoro di routine. Finché, dall'incontro con una ragazza sconosciuta, inizia per lui la scoperta di un sentimento e di una vita diversa, un mondo di luce non ancora offuscato dalle tenebre della imperante società tecnologica.\
    ")
}

fn saggio_erotico() -> Book {
    Book::new().with_title("Saggio Erotico sulla Fine del Mondo").with_npages(276).with_read_pages(240).with_cover_path("saggio-erotico-sulla-fine-del-mondo.jpg").with_author("Barbascura X").with_description("\
