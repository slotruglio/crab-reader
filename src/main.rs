...
use components::book::book_derived_lenses::{rich_text};
...
fn build_single_view() -> impl Widget<AppState> {
    ...
    Scroll::new(
        ...
        .lens(AppState::book.then(rich_text))
        ).vertical()
...
fn build_ui() -> impl Widget<AppState> {
    ...
    let rich_text_widget = Scroll::new(Label::raw().lens(AppState::book.then(rich_text)));
